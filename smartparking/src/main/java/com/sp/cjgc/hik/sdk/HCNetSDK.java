...
public interface HCNetSDK extends Library {
     ...
     */
    static final String prodUrl = "/smartParking/backend/linux-hksdk";

    // 加载SDK
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(prodUrl + "/libhcnetsdk.so", HCNetSDK.class);
...
//入侵参数
    ...
    */
    ...
    //重写LED屏幕显示参数
    public static class NET_DVR_LED_CONTENT_CFG extends Structure {
        public int dwSize;//结构体大小
        public byte[] sLEDContent = new byte[MAX_LED_INFO_LEN/*512*/]; // LED显示内容
        public byte byContentAct;//运动模式：1- 静态，2- 快速打出，3- 向左移动，4- 向右移动，5- 向上移动，6- 向下移动
        public byte byContentSpeed;//运动速度：1~24（1最快，24最慢）
        public byte byContentStayTime;//停留时间（0~127.5s，精度0.5s），客户端赋值的时候为实际值*2，即传送数值范围：0~255，设备端在使用的时候再除以2
...
14-白炽灯 (IncandescentLamp)，15-暖光灯(Warm Light Lamp)，16-自然光(Natural Light) */
    ...
    public static interface FDrawFun extends Callback {
        public void invoke(int lRealHandle, int hDc, int dwUser);
    }
    ...
    int NET_DVR_PlayBackByName(int lUserID, String sPlayBackFileName, int hWnd);
